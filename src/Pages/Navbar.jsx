        ...
        <div className="flex justify-between items-center h-[72px] py-2">
          <div className="flex items-center space-x-3 group cursor-pointer">
            <div className="relative">
              <div className="w-12 h-12 bg-gradient-to-br from-purple-500 via-blue-500 to-indigo-600 rounded-2xl flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform">
                <ShoppingBag className="w-7 h-7 text-white group-hover:rotate-12 transition-transform" />
              </div>
              <div className="absolute -top-1 -right-1 w-4 h-4 bg-yellow-400 rounded-full animate-pulse"></div>
            </div>
            <div className="flex flex-col">
              <span className="text-2xl font-black bg-gradient-to-r from-purple-600 via-blue-600 to-indigo-600 bg-clip-text text-transparent">StudentBazaar</span>
  ...
  <NavLink to="/" icon={Home}>Home</NavLink>
            ...
            {authUserToken && (
              <>
                ...
                <button
                  onClick={logoutUser}
                  className="group flex items-center space-x-2 bg-red-500 hover:bg-red-600 text-white px-5 py-2 rounded-xl text-sm font-medium shadow-md hover:shadow-xl ml-2"
                >
            ...
            {!authUserToken && !authAdminToken && (
              <>
                ...
                <Link to="/register" className="group flex items-center space-x-2 bg-purple-500 hover:bg-purple-600 text-white px-5 py-2 rounded-xl text-sm font-medium shadow-md hover:shadow-xl ml-2">
          ...
          <div className="md:hidden">
            <button onClick={toggleMobileMenu} className="p-3 rounded-xl text-gray-700 hover:text-purple-600 hover:bg-purple-100 transition-transform">
      ...
      <div className={`md:hidden transition-all duration-500 ease-in-out ${isMobileMenuOpen ? 'max-h-[600px] opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}>
        <div className="px-4 pt-4 pb-6 space-y-3 bg-purple-50 border-t border-purple-100">
{(authUserToken || (!authUserToken && !authAdminToken)) && (
  <MobileNavLink to="/" icon={Home} onClick={toggleMobileMenu}>Home</MobileNavLink>
          ...
          {authUserToken && (
            <>
              ...
              <button onClick={logoutUser} className="w-full text-left flex items-center space-x-3 bg-red-500 hover:bg-red-600 text-white px-4 py-3 rounded-xl font-medium">
                <LogOut size={20} />
                <span>Logout</span>
              </button>
            </>
          ...
          {authAdminToken && (
            <>
              ...
              <button onClick={logoutAdmin} className="w-full text-left flex items-center space-x-3 bg-red-500 hover:bg-red-600 text-white px-4 py-3 rounded-xl font-medium">
                <LogOut size={20} />
                <span>Logout</span>
              </button>
            </>
          ...
          {!authUserToken && !authAdminToken && (
            <>
              ...
              <Link to="/register" onClick={toggleMobileMenu} className="w-full flex items-center space-x-3 bg-purple-500 hover:bg-purple-600 text-white px-4 py-3 rounded-xl font-medium">
                <UserPlus size={20} />
                <span>Register</span>
              </Link>
            </>
